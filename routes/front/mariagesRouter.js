...
router.get('/:breedSlug/:slug', (req, res) => {
    const userId = 10;
    const { slug, breedSlug } = req.params;
        ...
        db.query(queryMarriageDetail, [slug, userId], (err, result) => {
            ...
            Promise.all([
                new Promise((resolve, reject) => {
                    db.query(queryPuppies, [marriage.id], (err, result) => {
            ...
            ]).then(([puppiesResult, socialLinks]) => {
                ...
                res.render('mariage_detail', {
                    marriage: {
                        title: `${marriage.male_name} x ${marriage.female_name}`,
                        ...
                        image: {
                            path: marriage.marriage_optimized_image_path ?
                                `/uploads/optimized/${marriage.marriage_optimized_image_path}` :
                                `/uploads/${marriage.marriage_image_path}`
                        }
                    },
                    father: {
                        ...
                        image: {
                            path: marriage.male_optimized_image_path ?
                                `/uploads/optimized/${marriage.male_optimized_image_path}` :
                                `/uploads/${marriage.male_image_path}`,
                    ...
                    mother: {
                        ...
                        image: {
                            path: marriage.female_optimized_image_path ?
                                `/uploads/optimized/${marriage.female_optimized_image_path}` :
                                `/uploads/${marriage.female_image_path}`,
...
router.get('/:breedSlug', (req, res) => {
    ...
    Promise.all([
        new Promise((resolve, reject) => {
            db.query(queryBreeds, [userId], (err, result) => {
                if (err) reject(err); else resolve(result);
            });
        }),
        new Promise((resolve, reject) => {
            db.query(countQuery, [userId, breedSlug], (err, result) => {
    ...
    ]).then(([breeds, totalItems, socialLinks]) => {
